  ...
  while (labels) {
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      if (axisInfo.isLabelRotated(levelIdx)) {
        ...
        if (!isTickInside && dvt.TextUtils.guessTextDimensions(label).h - 1 > availSpace.h) // -1 to prevent rounding error (bug 18960904)
          continue;
  ...
  if (axisInfo instanceof dvt.TimeAxisInfo) {
    ...
    if (lv2Labels != null) {
      for (i = 0; i < lv2Labels.length; i++) {
        ...
        if (label == null)
          continue;
        if (dvt.TextUtils.guessTextDimensions(label).h - 1 > availSpace.h) // -1 to prevent rounding error (bug 18960904)
          continue;
...
DvtAxisRenderer._renderLabelsVert = function(axis, axisInfo, availSpace) {
  ...
  var formatLabelVert = function(label, index) {
    ...
    if (isHierarchical && dvt.TextUtils.getMaxTextDimensions(labels).w - 1 > availSpace.w) // -1 to prevent rounding error (bug 18960904)
      return;
